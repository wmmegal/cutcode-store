...
@section('content')
    <x-forms.auth-form title="Регистрация" action="{{ route('signUp') }}">
        <x-forms.input placeholder="Имя" name="name" :is-error="$errors->has('name')" value="{{ old('name') }}" required/>
        @error('name')
        <x-forms.error>
            {{ $message }}
        </x-forms.error>
