...
@section('content')
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-5 gap-3 sm:gap-4 md:gap-5 mt-8">
            @foreach($categories as $item)
                <a href="{{ route('catalog', $item->slug) }}"
                   class="p-3 sm:p-4 2xl:p-6 rounded-xl {{ $category->slug === $item->slug ? 'bg-pink' : 'bg-card' }} hover:bg-pink text-xxs sm:text-xs lg:text-sm text-white font-semibold"
                >
                    {{ $item->title }}
                </a>
            @endforeach
    ...
    <section class="mt-16 lg:mt-24">
        <!-- Section heading -->
        <h2 class="text-lg lg:text-[42px] font-black">Catalog</h2>
        ...
        <div class="flex flex-col lg:flex-row gap-12 lg:gap-6 2xl:gap-8 mt-8">
            ...
            <aside class="basis-2/5 xl:basis-1/4">
                <form action="{{ route('catalog', $category) }}"
                    ...
                    <div>
                        <button type="submit" class="w-full !h-16 btn btn-pink">Search</button>
                    </div>

                    @if(request('filters'))
                        <div>
                            <a href="{{ route('catalog', $category) }}" class="w-full !h-16 btn btn-outline">Reset filters</a>
                        </div>
                    @endif
                </form>
            </aside>
                ...
                <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
                    <div class="flex items-center gap-4">
                        ...
                        <div class="text-body text-xxs sm:text-xs">
                            Found: {{ $products->total() }} {{ str('product')->plural($products->total()) }}
                        </div>
                    </div>
                    <div x-data="{sort: '{{ filter_url($category, ['sort' => request('sort')]) }}'}"
                         class="flex flex-col sm:flex-row sm:items-center gap-3">
                        <span class="text-body text-xxs sm:text-xs">Sort:</span>
                        ...
                        <select
                            ...
                            <option value="{{ filter_url($category, ['sort' => '']) }}" class="text-dark">default
                            </option>
